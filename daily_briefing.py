...
"""
    ...
    def parse_article_date(self, date_string, source_name="Unknown"):
        ...
        try:
            ...
            # Remove timezone abbreviations that cause issues
            timezone_abbrevs = [' GMT', ' UTC', ' EST', ' PST', ' CST', ' MST', ' EDT', ' PDT', ' CDT', ' MDT']
            for tz in timezone_abbrevs:
                cleaned_date = cleaned_date.replace(tz, '')
            
            # Remove trailing timezone offsets like +0000, -0500
            cleaned_date = re.sub(r'\s*[+-]\d{4}
...
if __name__ == "__main__":
    ...
    briefing_system.run_daily_briefing(), '', cleaned_date)
            ...
            if parsed_date:
                ...
                max_future = now + timedelta(hours=1)  # Allow small clock differences
                
                if max_past <= parsed_date <= max_future:
                    return parsed_date
                else:
